...
if (isWorker() || (!isVercel())) {
  try {
    ...
    logger.info('✅ BullMQ Redis connection created');
  } catch (error) {
...
export const cancelAllReminders = async (bookingId) => {
  try {
    ...
    // Worker mode logic
    if (!bookingQueue) {
      throw new Error('Booking queue not available');
    }
...
if (isWorker() || (!isVercel() && bullmqRedis)) {
  try {
    bookingQueue = new Queue('bookingQueue', {
      connection: bullmqRedis, // Using BullMQ-compatible connection
      defaultJobOptions: {
        attempts: 3,
        backoff: {
          type: 'exponential',
    ...
    logger.info('✅ Booking queue created successfully');
    ...
    if (isWorker()) {
      ...
      logger.info('✅ Booking queue events created successfully');
    }
  } catch (error) {
    logger.error('❌ Failed to create booking queue:', error);
    bookingQueue = null;
    bookingQueueEvents = null;
  }
}
...
export const startBookingQueue = async () => {
  if (!bookingQueue) {
    logger.info('No booking queue to start (Vercel mode or connection failed)');
    return true;
  }
